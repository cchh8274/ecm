...
import cn.kanmars.ecm.dao.TblSysSequenceMapper;
import cn.kanmars.ecm.entity.TblSysSequence;
 ...
 * 使用场景：有数据库访问，需要数据生成序列的系统，例如ecm-admin,ecm-service,ecm-task等，严禁ecm-web和ecm-zuul使用
 */
@Transactional
public class SysSequenceUtils implements ApplicationContextAware {
    ...
    @Transactional(value="ecm-txManager",propagation = Propagation.REQUIRES_NEW,rollbackFor = Exception.class)
    ...
    private boolean createNewSequence_(String keyValue){
        ...
        sequence.setCurrValue(1l);
        sequence.setCurrValue(1l);
        sequence.setBatchSize(1l);
        sequence.setBatchSize(1l);
    ...
    static class IdSpace implements Cloneable, Serializable {
        ...
        public void setBatchSize(long pBatchSize) {
    ...
    public static long createSimpleId(){
        return IDGeneratorTools.createSimpleLong();
    }
}
