...
import cn.com.xbase.frame.cache.IdValue;
 ...
 *
 * 使用场景：有数据库访问，需要数据字典的系统，例如ecm-admin,ecm-service,ecm-task等，严禁ecm-web和ecm-zuul使用
 */
public class SysDicUtils implements ApplicationContextAware {
    ...
    /** 精确到第三层级的字典 */
    protected static final ConcurrentHashMap<String, ConcurrentHashMap<String, ConcurrentHashMap<String, TblSysDic>>> map = new ConcurrentHashMap<String, ConcurrentHashMap<String, ConcurrentHashMap<String, TblSysDic>>>();
    /** 精确到第二层级的列表字典 */
    protected static final ConcurrentHashMap<String, ConcurrentHashMap<String, List<TblSysDic>>> map_list = new ConcurrentHashMap<String, ConcurrentHashMap<String, List<TblSysDic>>>();
    ...
    public UserDicGetter userDicGetter_;

    public static UserDicGetter userDicGetter;
    ...
    public synchronized void init(){
        ...
        try{
            if(applicationContext.containsBean("userDicGetter")){
                userDicGetter_ = (UserDicGetter)applicationContext.getBean("userDicGetter");
            }
        }catch (Exception e){
            e.printStackTrace();
        }
        try{
            ...
            userDicGetter = userDicGetter_;
            if(userDicGetter == null){
                logger.info("无自定义数据字典加载器");
            }
            ...
            new Thread(new Runnable() {
                public void run() {
                    ...
                    while(true){
                        long now = System.currentTimeMillis();
                        try {
                             Thread.sleep(60000);//每30秒刷新一次缓存
                        } catch (InterruptedException e) {
                            logger.error("数据字典缓存清理",e);
                        }
                        cleanTask.run();
                    }
                }
    ...
    public static String getValue(String l1Code, String l2Code, String codeParam) {
        ...
        if (tblSysDic == null) {
            if(userDicGetter==null){
                ...
                tblSysDic = userDicGetter.getUserDic(l1Code,l2Code,codeParam);
                if(tblSysDic==null){
                    return null;
                }
            }
    ...
    public static List<TblSysDic> getValueList(String l1Code, String l2Code) {
        ...
        if (list == null || list.size() == 0) {
            if(userDicGetter==null) {
                ...
                list = userDicGetter.getUserDicList(l1Code,l2Code);
                if (list == null || list.size() == 0) {
                    return new ArrayList<TblSysDic>();
                }
            }
    ...
    public interface UserDicGetter{
    ...
    public class CleanTask {
        public void run() {
            String datetime = DateFormatUtils.format(new Date(), "yyyyMMddHHmmss");
            {
                //清理map
                //protected static final ConcurrentHashMap<String, ConcurrentHashMap<String, ConcurrentHashMap<String, TblSysDic>>> map = new ConcurrentHashMap<String, ConcurrentHashMap<String, ConcurrentHashMap<String, TblSysDic>>>();
                HashSet<String> e1_keys = new HashSet<String>();
                for (Entry<String, ConcurrentHashMap<String, ConcurrentHashMap<String, TblSysDic>>> e1 : map.entrySet()) {
                    HashSet<String> e2_keys = new HashSet<String>();
                    for (Entry<String, ConcurrentHashMap<String, TblSysDic>> e2 : e1.getValue().entrySet()) {
                        HashSet<String> e3_keys = new HashSet<String>();
                        for (Entry<String, TblSysDic> e3 : e2.getValue().entrySet()) {
                            TblSysDic tblSysDic = e3.getValue();
                            String expireTime = DateUtils.countTime(tblSysDic.getUpTime(), tblSysDic.getLiveCount()+60);//过期后60s清理
                            if (expireTime.compareTo(datetime) >= 0) {// 例如，内存中是20151231000000而datetime是20150101000000，则不过期
                                // 未超时
                            } else {
                                // 已超时
                                e3_keys.add(e3.getKey());
                            }
                        }
                        for (String key : e3_keys) {
                            logger.debug("CleanThread-map清理[l1Code="+e1.getKey()+"][l2Code="+e2.getKey()+"][codeParam="+key+"][codeValue="+e2.getValue().get(key)+"] is remove");
                            e2.getValue().remove(key);
                        }
                        if (e2.getValue().isEmpty()) {
                            e2_keys.add(e2.getKey());
                        }
                    }
                    for (String key : e2_keys) {
                        logger.debug("CleanThread-map清理[l1Code="+e1.getKey()+"][l2Code="+key+"] is remove");
                        e1.getValue().remove(key);
                    }
                    if (e1.getValue().isEmpty()) {
                        e1_keys.add(e1.getKey());
                    }
                }
                for (String key : e1_keys) {
                    logger.debug("CleanThread-map清理[l1Code="+key+"] is remove");
                    map.remove(key);
                }
            }

            {
                ...
                //protected static final ConcurrentHashMap<String, ConcurrentHashMap<String, List<TblSysDic>>> map_list = new ConcurrentHashMap<String, ConcurrentHashMap<String, List<TblSysDic>>>();
                HashSet<String> e1_keys = new HashSet<String>();
                for (Entry<String, ConcurrentHashMap<String, List<TblSysDic>>> e1 : map_list.entrySet()) {
                    HashSet<String> e2_keys = new HashSet<String>();
                    for (Entry<String, List<TblSysDic>> e2 : e1.getValue().entrySet()) {
                        boolean isExpire = false;
                        for (TblSysDic tblSysDic : e2.getValue()) {
                            String expireTime = DateUtils.countTime(tblSysDic.getUpTime(), tblSysDic.getLiveCount()+60);//过期后60s清理
                            if (expireTime.compareTo(datetime) >= 0) {// 例如，内存中是20151231000000而datetime是20150101000000，则不过期
                                // 未超时
                            } else {
                    ...
                    for (String key : e2_keys) {
                        logger.debug("CleanThread-map_list清理[l1Code="+e1.getKey()+"][l2Code="+key+"] is remove");
                        e1.getValue().remove(key);
                    }
                    if (e1.getValue().isEmpty()) {
                        e1_keys.add(e1.getKey());
                ...
                for (String key : e1_keys) {
                    logger.debug("CleanThread-map_list清理[l1Code="+key+"] is remove");
                    map_list.remove(key);
                }
            }
        }
