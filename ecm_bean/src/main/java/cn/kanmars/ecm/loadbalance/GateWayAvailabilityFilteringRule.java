 ...
 * Created by kanmars on 2017/8/2.
 */
public class GateWayAvailabilityFilteringRule extends AvailabilityFilteringRule {

    private static Logger log = LoggerFactory.getLogger(GateWayAvailabilityFilteringRule.class);
    ...
    /**
     * 配置信息；
     * 当前HOST 和筛选模式(soft模式，hard模式) 目标HOST列表。
     * 例如:
     *  10.126.53.194::soft::10.126.53.196:10,10.126.53.197:10,...........;
     *  10.126.*::hard::10.126.53.196:10,10.126.53.197:10;
     *  10.126.[1-25].*::hard::
     *  如果是soft模式，则在原server列表上加强
     *  如果是hard模式，则根据列表，在server清单上选择.......
     *
     */
    private List<RouteEntry> propertiesInfo = new ArrayList<RouteEntry>();

    private String properties = "";

    private AtomicInteger nextServerCyclicCounter;

    public GateWayAvailabilityFilteringRule(){
        super();
        ...
        try{
            //TODO 此类可以增加新功能，例如从全局配置中获取字符串，但用途不大
    ...
    public Server choose(Object key) {
        ...
        while (server == null && count++ < 10) {
            ...
            parseRouters();
            //根据权重算法重置server列表
            reachableServers = processWithWeight(reachableServers);
            //根据权重算法重置server列表
            allServers = processWithWeight(allServers);
     ...
     * @return
     */
    private List<Server> processWithWeight(List<Server> servers){
