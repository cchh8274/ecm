...
{_this.attr("type","date");}
    ...
    {iilast="<span class='itime'>"+Math.abs(diff)+"</span> ";if(diff>0)
        ...
        var second = "00";
        if(_this.val().trim().lastIndexOf(":")==_this.val().trim().length-3){
            var sec = _this.val().trim().substr(_this.val().trim().lastIndexOf(":")+1,2);
            if(sec.substr(0,1)>='0' && sec.substr(0,1)<='5' && sec.substr(1,1)>='0' && sec.substr(1,1)<='9'){
                second = sec;
            }
        }
        var val=config
            .format.replace('yyyy',getFullYear)
