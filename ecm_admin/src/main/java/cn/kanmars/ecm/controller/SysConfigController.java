...
public class SysConfigController extends AdvancedAjaxBaseController  {
    ...
    private Map<String, Object> afterLogicpreCell(HttpServletRequest request, Map<String, Object> result) throws Exception {
            if (result.keySet().contains("menuMode")) {
                String key = "menuMode";
                Object value = result.get(key);
                if(value!=null&&StringUtils.isNotEmpty(value.toString())){
                    try {
                        String menuMode_name = null;
                        JSONObject json = JSONObject.fromObject("{'020':'关闭模式','010':'展开模式'}");
                        menuMode_name = json.get(value.toString())==null?value.toString():(String)json.get(value.toString());
                        result.put("menuMode_name", menuMode_name);
                    } catch(Exception ex) {
                        logger.error("格式转化失败["+key+"]["+value+"]", ex);
                        throw new Exception("格式转化失败["+key+"]["+value+"]");
                    }
