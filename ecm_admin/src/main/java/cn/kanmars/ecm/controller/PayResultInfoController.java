...
import cn.kanmars.ecm.logic.PayResultInfoLogic;
 ...
 */
@Controller
@RequestMapping("/payResultInfo")
public class PayResultInfoController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("PayResultInfoController");

    @Autowired
    private PayResultInfoLogic payResultInfoLogic;

    @RequestMapping("/payResultInfoView.dhtml")
    public String init() {
        return "payresultinfo/payresultinfo";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        TblPayResultInfo tblPayResultInfo = new TblPayResultInfo();
        tblPayResultInfo.setId(Integer.parseInt(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblPayResultInfo = payResultInfoLogic.queryPayResultInfo(tblPayResultInfo);
            logger.info("查询结果:tblPayResultInfo:" + (tblPayResultInfo != null ? JSONObject.fromObject(tblPayResultInfo).toString() : "null"));
            Map result = new HashMap();
            Map tblPayResultInfo_map = afterLogic(request, JSONObject.fromObject(tblPayResultInfo));
            result.put("tblPayResultInfo", tblPayResultInfo_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblPayResultInfo tblPayResultInfo = new TblPayResultInfo();
            try {
                MapObjTransUtils.mapToObj(tblPayResultInfo, paramMap);
            } catch (Exception e) {
            ...
            logger.info("传入参数:tblPayResultInfo:" + JSONObject.fromObject(tblPayResultInfo).toString());
            int count = payResultInfoLogic.insertPayResultInfo(tblPayResultInfo);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblPayResultInfo:" + JSONObject.fromObject(tblPayResultInfo).toString());
            int count = payResultInfoLogic.updatePayResultInfo(tblPayResultInfo);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblPayResultInfo tblPayResultInfo = new TblPayResultInfo();
                tblPayResultInfo.setId(Integer.parseInt(s));// 联合主键用逗号分割
                try {
                    count += payResultInfoLogic.deletePayResultInfo(tblPayResultInfo);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = payResultInfoLogic.queryPagePayResultInfo(paramMap);
