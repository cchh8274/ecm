...
import cn.kanmars.ecm.logic.TaskDetailLogLogic;
...
public class TaskDetailLogController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("TaskDetailLogController");

    @Autowired
    private TaskDetailLogLogic taskDetailLogLogic;

    @RequestMapping("/taskDetailLogView.dhtml")
    public String init() {
        return "taskdetaillog/taskdetaillog";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        tblTaskDetailLog.setId(Long.parseLong(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblTaskDetailLog = taskDetailLogLogic.queryTaskDetailLog(tblTaskDetailLog);
            logger.info("查询结果:tblTaskDetailLog:" + (tblTaskDetailLog != null ? JSONObject.fromObject(tblTaskDetailLog).toString() : "null"));
            Map result = new HashMap();
            Map tblTaskDetailLog_map = afterLogic(request, JSONObject.fromObject(tblTaskDetailLog));
            result.put("tblTaskDetailLog", tblTaskDetailLog_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblTaskDetailLog tblTaskDetailLog = new TblTaskDetailLog();
            try {
                MapObjTransUtils.mapToObj(tblTaskDetailLog, paramMap);
            } catch (Exception e) {
            ...
            logger.info("传入参数:tblTaskDetailLog:" + JSONObject.fromObject(tblTaskDetailLog).toString());
            int count = taskDetailLogLogic.insertTaskDetailLog(tblTaskDetailLog);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblTaskDetailLog:" + JSONObject.fromObject(tblTaskDetailLog).toString());
            int count = taskDetailLogLogic.updateTaskDetailLog(tblTaskDetailLog);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblTaskDetailLog tblTaskDetailLog = new TblTaskDetailLog();
                tblTaskDetailLog.setId(Long.parseLong(s));// 联合主键用逗号分割
                try {
                    count += taskDetailLogLogic.deleteTaskDetailLog(tblTaskDetailLog);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = taskDetailLogLogic.queryPageTaskDetailLog(paramMap);
    ...
    protected HashMap<String, Object> beferLogic(HttpServletRequest request, Map<String, Object> obj) throws Exception {
        ...
        for (Map.Entry<String, Object> e : obj.entrySet()) {
            ...
            if (key.equals("beginTime") && StringUtils.isNotEmpty(value)) {
                try {
                    ...
                    result.put("beginTime", dbDateStr);
                } catch(Exception ex) {
                    logger.error("格式转化失败["+key+"]["+value+"]", ex);
                    throw new Exception("格式转化失败["+key+"]["+value+"]");
                }
            }
            if (key.equals("endTime") && StringUtils.isNotEmpty(value)) {
                try {
                    java.text.SimpleDateFormat sdf_page = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                    java.text.SimpleDateFormat sdf_db = new java.text.SimpleDateFormat("yyyyMMddHHmmss");
                    String dbDateStr=sdf_db.format(sdf_page.parse(value));
                    result.put("endTime", dbDateStr);
                } catch(Exception ex) {
                    logger.error("格式转化失败["+key+"]["+value+"]", ex);
                    throw new Exception("格式转化失败["+key+"]["+value+"]");
                }
    ...
    private Map<String, Object> afterLogicpreCell(HttpServletRequest request, Map<String, Object> result) throws Exception {
            ...
            if (result.keySet().contains("beginTime")) {
            ...
            if (result.keySet().contains("endTime")) {
                String key = "endTime";
                Object value = result.get(key);
                if(StringUtils.isNotEmpty((String)value)){
                    try {
