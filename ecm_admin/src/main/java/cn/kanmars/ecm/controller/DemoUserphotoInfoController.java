...
import cn.kanmars.ecm.logic.DemoUserphotoInfoLogic;
 ...
 */
@Controller
@RequestMapping("/demoUserphotoInfo")
public class DemoUserphotoInfoController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("DemoUserphotoInfoController");

    @Autowired
    private DemoUserphotoInfoLogic demoUserphotoInfoLogic;

    @RequestMapping("/demoUserphotoInfoView.dhtml")
    public String init() {
        return "demouserphotoinfo/demouserphotoinfo";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        TblDemoUserphotoInfo tblDemoUserphotoInfo = new TblDemoUserphotoInfo();
        tblDemoUserphotoInfo.setId(Long.parseLong(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblDemoUserphotoInfo = demoUserphotoInfoLogic.queryDemoUserphotoInfo(tblDemoUserphotoInfo);
            logger.info("查询结果:tblDemoUserphotoInfo:" + (tblDemoUserphotoInfo != null ? JSONObject.fromObject(tblDemoUserphotoInfo).toString() : "null"));
            Map result = new HashMap();
            Map tblDemoUserphotoInfo_map = afterLogic(request, JSONObject.fromObject(tblDemoUserphotoInfo));
            result.put("tblDemoUserphotoInfo", tblDemoUserphotoInfo_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblDemoUserphotoInfo tblDemoUserphotoInfo = new TblDemoUserphotoInfo();
            try {
                MapObjTransUtils.mapToObj(tblDemoUserphotoInfo, paramMap);
            } catch (Exception e) {
                logger.error("对象转化异常", e);
            }
            try {
                String pk0 = SysSequenceUtils.generateStringId("tbl_demo_userphoto_info");
                tblDemoUserphotoInfo.setId(Long.parseLong(pk0));// 联合主键用逗号分割
            } catch (Exception e) {
                logger.error("对象转化异常", e);
            }
            logger.info("传入参数:tblDemoUserphotoInfo:" + JSONObject.fromObject(tblDemoUserphotoInfo).toString());
            int count = demoUserphotoInfoLogic.insertDemoUserphotoInfo(tblDemoUserphotoInfo);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblDemoUserphotoInfo:" + JSONObject.fromObject(tblDemoUserphotoInfo).toString());
            int count = demoUserphotoInfoLogic.updateDemoUserphotoInfo(tblDemoUserphotoInfo);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblDemoUserphotoInfo tblDemoUserphotoInfo = new TblDemoUserphotoInfo();
                tblDemoUserphotoInfo.setId(Long.parseLong(s));// 联合主键用逗号分割
                try {
                    count += demoUserphotoInfoLogic.deleteDemoUserphotoInfo(tblDemoUserphotoInfo);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = demoUserphotoInfoLogic.queryPageDemoUserphotoInfo(paramMap);
    ...
    private Map<String, Object> afterLogicpreCell(HttpServletRequest request, Map<String, Object> result) throws Exception {
            ...
            if (result.keySet().contains("photo")) {
                String key = "photo";
                Object value = result.get(key);
                if(value != null){
                    //文件编号如果是long或者BigDecimal时转化为字符串，防止javascript不支持大数异常
                    result.put(key, value.toString());
                }
            }
            //*********************************************************
