...
import cn.kanmars.ecm.logic.AreaInfoLogic;
...
public class AreaInfoController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("AreaInfoController");

    @Autowired
    private AreaInfoLogic areaInfoLogic;

    @RequestMapping("/areaInfoView.dhtml")
    public String init() {
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        tblAreaInfo.setId(Integer.parseInt(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblAreaInfo = areaInfoLogic.queryAreaInfo(tblAreaInfo);
            logger.info("查询结果:tblAreaInfo:" + (tblAreaInfo != null ? JSONObject.fromObject(tblAreaInfo).toString() : "null"));
            Map result = new HashMap();
            Map tblAreaInfo_map = afterLogic(request, JSONObject.fromObject(tblAreaInfo));
            result.put("tblAreaInfo", tblAreaInfo_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblAreaInfo tblAreaInfo = new TblAreaInfo();
            try {
                MapObjTransUtils.mapToObj(tblAreaInfo, paramMap);
            } catch (Exception e) {
            ...
            logger.info("传入参数:tblAreaInfo:" + JSONObject.fromObject(tblAreaInfo).toString());
            int count = areaInfoLogic.insertAreaInfo(tblAreaInfo);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblAreaInfo:" + JSONObject.fromObject(tblAreaInfo).toString());
            int count = areaInfoLogic.updateAreaInfo(tblAreaInfo);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblAreaInfo tblAreaInfo = new TblAreaInfo();
                tblAreaInfo.setId(Integer.parseInt(s));// 联合主键用逗号分割
                try {
                    count += areaInfoLogic.deleteAreaInfo(tblAreaInfo);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = areaInfoLogic.queryPageAreaInfo(paramMap);
