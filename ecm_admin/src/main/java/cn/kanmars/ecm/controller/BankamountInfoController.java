...
import cn.kanmars.ecm.logic.BankamountInfoLogic;
 ...
 */
@Controller
@RequestMapping("/bankamountInfo")
public class BankamountInfoController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("BankamountInfoController");

    @Autowired
    private BankamountInfoLogic bankamountInfoLogic;

    @RequestMapping("/bankamountInfoView.dhtml")
    public String init() {
        return "bankamountinfo/bankamountinfo";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        TblBankamountInfo tblBankamountInfo = new TblBankamountInfo();
        tblBankamountInfo.setId(Integer.parseInt(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblBankamountInfo = bankamountInfoLogic.queryBankamountInfo(tblBankamountInfo);
            logger.info("查询结果:tblBankamountInfo:" + (tblBankamountInfo != null ? JSONObject.fromObject(tblBankamountInfo).toString() : "null"));
            Map result = new HashMap();
            Map tblBankamountInfo_map = afterLogic(request, JSONObject.fromObject(tblBankamountInfo));
            result.put("tblBankamountInfo", tblBankamountInfo_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblBankamountInfo tblBankamountInfo = new TblBankamountInfo();
            try {
                MapObjTransUtils.mapToObj(tblBankamountInfo, paramMap);
            } catch (Exception e) {
            ...
            logger.info("传入参数:tblBankamountInfo:" + JSONObject.fromObject(tblBankamountInfo).toString());
            int count = bankamountInfoLogic.insertBankamountInfo(tblBankamountInfo);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblBankamountInfo:" + JSONObject.fromObject(tblBankamountInfo).toString());
            int count = bankamountInfoLogic.updateBankamountInfo(tblBankamountInfo);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblBankamountInfo tblBankamountInfo = new TblBankamountInfo();
                tblBankamountInfo.setId(Integer.parseInt(s));// 联合主键用逗号分割
                try {
                    count += bankamountInfoLogic.deleteBankamountInfo(tblBankamountInfo);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = bankamountInfoLogic.queryPageBankamountInfo(paramMap);
