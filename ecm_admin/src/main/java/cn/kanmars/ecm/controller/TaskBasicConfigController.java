...
import cn.kanmars.ecm.logic.TaskBasicConfigLogic;
import cn.kanmars.ecm.entity.TblTaskBasicConfig;
...
public class TaskBasicConfigController extends AdvancedAjaxBaseController  {
    ...
    private Map<String, Object> afterLogicpreCell(HttpServletRequest request, Map<String, Object> result) throws Exception {
            if (result.keySet().contains("runStatus")) {
                ...
                if(value!=null&&StringUtils.isNotEmpty(value.toString())){
                    try {
                        ...
                        JSONObject json = JSONObject.fromObject("{'0':'无限制执行','1':'同任务串行执行','2':'同任务执行中抛弃新来任务'}");
            ...
            if (result.keySet().contains("status")) {
                ...
                if(value!=null&&StringUtils.isNotEmpty(value.toString())){
                    try {
                        ...
                        JSONObject json = JSONObject.fromObject("{'1':'启用','2':'禁用'}");
            ...
            if (result.keySet().contains("logFlg")) {
                String key = "logFlg";
                Object value = result.get(key);
                if(value!=null&&StringUtils.isNotEmpty(value.toString())){
                    try {
                        String logFlg_name = null;
                        JSONObject json = JSONObject.fromObject("{'020':'禁用','010':'启用'}");
                        logFlg_name = json.get(value.toString())==null?value.toString():(String)json.get(value.toString());
                        result.put("logFlg_name", logFlg_name);
                    } catch(Exception ex) {
                        logger.error("格式转化失败["+key+"]["+value+"]", ex);
                        throw new Exception("格式转化失败["+key+"]["+value+"]");
                    }
            ...
            if (result.keySet().contains("continueFlg")) {
                String key = "continueFlg";
                Object value = result.get(key);
                if(value!=null&&StringUtils.isNotEmpty(value.toString())){
                    try {
                        String continueFlg_name = null;
                        JSONObject json = JSONObject.fromObject("{'020':'禁用','010':'启用'}");
                        continueFlg_name = json.get(value.toString())==null?value.toString():(String)json.get(value.toString());
                        result.put("continueFlg_name", continueFlg_name);
                    } catch(Exception ex) {
                        logger.error("格式转化失败["+key+"]["+value+"]", ex);
                        throw new Exception("格式转化失败["+key+"]["+value+"]");
                    }
