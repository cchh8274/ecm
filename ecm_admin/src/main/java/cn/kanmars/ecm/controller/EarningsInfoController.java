...
import cn.kanmars.ecm.logic.EarningsInfoLogic;
...
public class EarningsInfoController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("EarningsInfoController");

    @Autowired
    private EarningsInfoLogic earningsInfoLogic;

    @RequestMapping("/earningsInfoView.dhtml")
    public String init() {
        return "earningsinfo/earningsinfo";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        TblEarningsInfo tblEarningsInfo = new TblEarningsInfo();
        tblEarningsInfo.setId(Integer.parseInt(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblEarningsInfo = earningsInfoLogic.queryEarningsInfo(tblEarningsInfo);
            logger.info("查询结果:tblEarningsInfo:" + (tblEarningsInfo != null ? JSONObject.fromObject(tblEarningsInfo).toString() : "null"));
            Map result = new HashMap();
            Map tblEarningsInfo_map = afterLogic(request, JSONObject.fromObject(tblEarningsInfo));
            result.put("tblEarningsInfo", tblEarningsInfo_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblEarningsInfo tblEarningsInfo = new TblEarningsInfo();
            try {
                MapObjTransUtils.mapToObj(tblEarningsInfo, paramMap);
            } catch (Exception e) {
            ...
            logger.info("传入参数:tblEarningsInfo:" + JSONObject.fromObject(tblEarningsInfo).toString());
            int count = earningsInfoLogic.insertEarningsInfo(tblEarningsInfo);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblEarningsInfo:" + JSONObject.fromObject(tblEarningsInfo).toString());
            int count = earningsInfoLogic.updateEarningsInfo(tblEarningsInfo);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblEarningsInfo tblEarningsInfo = new TblEarningsInfo();
                tblEarningsInfo.setId(Integer.parseInt(s));// 联合主键用逗号分割
                try {
                    count += earningsInfoLogic.deleteEarningsInfo(tblEarningsInfo);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = earningsInfoLogic.queryPageEarningsInfo(paramMap);
