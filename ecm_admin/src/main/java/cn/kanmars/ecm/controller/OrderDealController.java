...
import cn.kanmars.ecm.logic.OrderDealLogic;
...
public class OrderDealController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("OrderDealController");

    @Autowired
    private OrderDealLogic orderDealLogic;

    @RequestMapping("/orderDealView.dhtml")
    public String init() {
        return "orderdeal/orderdeal";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        TblOrderDeal tblOrderDeal = new TblOrderDeal();
        tblOrderDeal.setId(Integer.parseInt(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblOrderDeal = orderDealLogic.queryOrderDeal(tblOrderDeal);
            logger.info("查询结果:tblOrderDeal:" + (tblOrderDeal != null ? JSONObject.fromObject(tblOrderDeal).toString() : "null"));
            Map result = new HashMap();
            Map tblOrderDeal_map = afterLogic(request, JSONObject.fromObject(tblOrderDeal));
            result.put("tblOrderDeal", tblOrderDeal_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblOrderDeal tblOrderDeal = new TblOrderDeal();
            try {
                MapObjTransUtils.mapToObj(tblOrderDeal, paramMap);
            } catch (Exception e) {
            ...
            logger.info("传入参数:tblOrderDeal:" + JSONObject.fromObject(tblOrderDeal).toString());
            int count = orderDealLogic.insertOrderDeal(tblOrderDeal);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblOrderDeal:" + JSONObject.fromObject(tblOrderDeal).toString());
            int count = orderDealLogic.updateOrderDeal(tblOrderDeal);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblOrderDeal tblOrderDeal = new TblOrderDeal();
                tblOrderDeal.setId(Integer.parseInt(s));// 联合主键用逗号分割
                try {
                    count += orderDealLogic.deleteOrderDeal(tblOrderDeal);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = orderDealLogic.queryPageOrderDeal(paramMap);
