...
import cn.kanmars.ecm.logic.UnionDemoSubInfoLogic;
 ...
 */
@Controller
@RequestMapping("/unionDemoSubInfo")
public class UnionDemoSubInfoController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("UnionDemoSubInfoController");

    @Autowired
    private UnionDemoSubInfoLogic unionDemoSubInfoLogic;

    @RequestMapping("/unionDemoSubInfoView.dhtml")
    public String init() {
        return "uniondemosubinfo/uniondemosubinfo";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        tblUnionDemoSubInfo.setDemoIdc(Long.parseLong(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblUnionDemoSubInfo = unionDemoSubInfoLogic.queryUnionDemoSubInfo(tblUnionDemoSubInfo);
            logger.info("查询结果:tblUnionDemoSubInfo:" + (tblUnionDemoSubInfo != null ? JSONObject.fromObject(tblUnionDemoSubInfo).toString() : "null"));
            Map result = new HashMap();
            Map tblUnionDemoSubInfo_map = afterLogic(request, JSONObject.fromObject(tblUnionDemoSubInfo));
            result.put("tblUnionDemoSubInfo", tblUnionDemoSubInfo_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            try {
                ...
                tblUnionDemoSubInfo.setDemoIdc(Long.parseLong(pk0));// 联合主键用逗号分割
                String pk1 = SysSequenceUtils.generateStringId("tbl_demo_info");
            } catch (Exception e) {
                logger.error("对象转化异常", e);
            }
            logger.info("传入参数:tblUnionDemoSubInfo:" + JSONObject.fromObject(tblUnionDemoSubInfo).toString());
            int count = unionDemoSubInfoLogic.insertUnionDemoSubInfo(tblUnionDemoSubInfo);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblUnionDemoSubInfo:" + JSONObject.fromObject(tblUnionDemoSubInfo).toString());
            int count = unionDemoSubInfoLogic.updateUnionDemoSubInfo(tblUnionDemoSubInfo);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblUnionDemoSubInfo tblUnionDemoSubInfo = new TblUnionDemoSubInfo();
                tblUnionDemoSubInfo.setDemoIdc(Long.parseLong(s));// 联合主键用逗号分割
                try {
                    count += unionDemoSubInfoLogic.deleteUnionDemoSubInfo(tblUnionDemoSubInfo);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = unionDemoSubInfoLogic.queryPageUnionDemoSubInfo(paramMap);
