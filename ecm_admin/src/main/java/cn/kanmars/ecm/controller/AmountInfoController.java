...
import cn.kanmars.ecm.logic.AmountInfoLogic;
 ...
 */
@Controller
@RequestMapping("/amountInfo")
public class AmountInfoController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("AmountInfoController");

    @Autowired
    private AmountInfoLogic amountInfoLogic;

    @RequestMapping("/amountInfoView.dhtml")
    public String init() {
        return "amountinfo/amountinfo";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        TblAmountInfo tblAmountInfo = new TblAmountInfo();
        tblAmountInfo.setId(Integer.parseInt(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblAmountInfo = amountInfoLogic.queryAmountInfo(tblAmountInfo);
            logger.info("查询结果:tblAmountInfo:" + (tblAmountInfo != null ? JSONObject.fromObject(tblAmountInfo).toString() : "null"));
            Map result = new HashMap();
            Map tblAmountInfo_map = afterLogic(request, JSONObject.fromObject(tblAmountInfo));
            result.put("tblAmountInfo", tblAmountInfo_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblAmountInfo tblAmountInfo = new TblAmountInfo();
            try {
                MapObjTransUtils.mapToObj(tblAmountInfo, paramMap);
            } catch (Exception e) {
            ...
            logger.info("传入参数:tblAmountInfo:" + JSONObject.fromObject(tblAmountInfo).toString());
            int count = amountInfoLogic.insertAmountInfo(tblAmountInfo);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblAmountInfo:" + JSONObject.fromObject(tblAmountInfo).toString());
            int count = amountInfoLogic.updateAmountInfo(tblAmountInfo);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblAmountInfo tblAmountInfo = new TblAmountInfo();
                tblAmountInfo.setId(Integer.parseInt(s));// 联合主键用逗号分割
                try {
                    count += amountInfoLogic.deleteAmountInfo(tblAmountInfo);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = amountInfoLogic.queryPageAmountInfo(paramMap);
