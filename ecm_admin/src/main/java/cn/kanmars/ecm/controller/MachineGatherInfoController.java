...
import cn.kanmars.ecm.logic.MachineGatherInfoLogic;
...
public class MachineGatherInfoController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("MachineGatherInfoController");

    @Autowired
    private MachineGatherInfoLogic machineGatherInfoLogic;

    @RequestMapping("/machineGatherInfoView.dhtml")
    public String init() {
        return "machinegatherinfo/machinegatherinfo";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        TblMachineGatherInfo tblMachineGatherInfo = new TblMachineGatherInfo();
        tblMachineGatherInfo.setId(Integer.parseInt(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblMachineGatherInfo = machineGatherInfoLogic.queryMachineGatherInfo(tblMachineGatherInfo);
            logger.info("查询结果:tblMachineGatherInfo:" + (tblMachineGatherInfo != null ? JSONObject.fromObject(tblMachineGatherInfo).toString() : "null"));
            Map result = new HashMap();
            Map tblMachineGatherInfo_map = afterLogic(request, JSONObject.fromObject(tblMachineGatherInfo));
            result.put("tblMachineGatherInfo", tblMachineGatherInfo_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblMachineGatherInfo tblMachineGatherInfo = new TblMachineGatherInfo();
            try {
                MapObjTransUtils.mapToObj(tblMachineGatherInfo, paramMap);
            } catch (Exception e) {
            ...
            logger.info("传入参数:tblMachineGatherInfo:" + JSONObject.fromObject(tblMachineGatherInfo).toString());
            int count = machineGatherInfoLogic.insertMachineGatherInfo(tblMachineGatherInfo);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblMachineGatherInfo:" + JSONObject.fromObject(tblMachineGatherInfo).toString());
            int count = machineGatherInfoLogic.updateMachineGatherInfo(tblMachineGatherInfo);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblMachineGatherInfo tblMachineGatherInfo = new TblMachineGatherInfo();
                tblMachineGatherInfo.setId(Integer.parseInt(s));// 联合主键用逗号分割
                try {
                    count += machineGatherInfoLogic.deleteMachineGatherInfo(tblMachineGatherInfo);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = machineGatherInfoLogic.queryPageMachineGatherInfo(paramMap);
