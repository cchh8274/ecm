...
import cn.kanmars.ecm.logic.MachineInfoLogic;
...
public class MachineInfoController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("MachineInfoController");

    @Autowired
    private MachineInfoLogic machineInfoLogic;

    @RequestMapping("/machineInfoView.dhtml")
    public String init() {
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        TblMachineInfo tblMachineInfo = new TblMachineInfo();
        tblMachineInfo.setId(jsonObject.get("id_key").toString().split(",")[0]);// 联合主键用逗号分割
        tblMachineInfo.setMachineName(jsonObject.get("id_key").toString().split(",")[1]);// 联合主键用逗号分割

        try {
            tblMachineInfo = machineInfoLogic.queryMachineInfo(tblMachineInfo);
            logger.info("查询结果:tblMachineInfo:" + (tblMachineInfo != null ? JSONObject.fromObject(tblMachineInfo).toString() : "null"));
            Map result = new HashMap();
            Map tblMachineInfo_map = afterLogic(request, JSONObject.fromObject(tblMachineInfo));
            result.put("tblMachineInfo", tblMachineInfo_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblMachineInfo tblMachineInfo = new TblMachineInfo();
            try {
                MapObjTransUtils.mapToObj(tblMachineInfo, paramMap);
            } catch (Exception e) {
            ...
            logger.info("传入参数:tblMachineInfo:" + JSONObject.fromObject(tblMachineInfo).toString());
            int count = machineInfoLogic.insertMachineInfo(tblMachineInfo);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblMachineInfo:" + JSONObject.fromObject(tblMachineInfo).toString());
            int count = machineInfoLogic.updateMachineInfo(tblMachineInfo);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblMachineInfo tblMachineInfo = new TblMachineInfo();
                tblMachineInfo.setId(s.split(",")[0]);// 联合主键用逗号分割
                tblMachineInfo.setMachineName(s.split(",")[1]);// 联合主键用逗号分割
                try {
                    count += machineInfoLogic.deleteMachineInfo(tblMachineInfo);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = machineInfoLogic.queryPageMachineInfo(paramMap);
