...
import cn.kanmars.ecm.logic.AmountDetailLogic;
...
public class AmountDetailController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("AmountDetailController");

    @Autowired
    private AmountDetailLogic amountDetailLogic;

    @RequestMapping("/amountDetailView.dhtml")
    public String init() {
        return "amountdetail/amountdetail";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        TblAmountDetail tblAmountDetail = new TblAmountDetail();
        tblAmountDetail.setId(Integer.parseInt(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblAmountDetail = amountDetailLogic.queryAmountDetail(tblAmountDetail);
            logger.info("查询结果:tblAmountDetail:" + (tblAmountDetail != null ? JSONObject.fromObject(tblAmountDetail).toString() : "null"));
            Map result = new HashMap();
            Map tblAmountDetail_map = afterLogic(request, JSONObject.fromObject(tblAmountDetail));
            result.put("tblAmountDetail", tblAmountDetail_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblAmountDetail tblAmountDetail = new TblAmountDetail();
            try {
                MapObjTransUtils.mapToObj(tblAmountDetail, paramMap);
            } catch (Exception e) {
            ...
            logger.info("传入参数:tblAmountDetail:" + JSONObject.fromObject(tblAmountDetail).toString());
            int count = amountDetailLogic.insertAmountDetail(tblAmountDetail);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblAmountDetail:" + JSONObject.fromObject(tblAmountDetail).toString());
            int count = amountDetailLogic.updateAmountDetail(tblAmountDetail);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblAmountDetail tblAmountDetail = new TblAmountDetail();
                tblAmountDetail.setId(Integer.parseInt(s));// 联合主键用逗号分割
                try {
                    count += amountDetailLogic.deleteAmountDetail(tblAmountDetail);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = amountDetailLogic.queryPageAmountDetail(paramMap);
