...
import cn.kanmars.ecm.logic.OrderInfoLogic;
...
public class OrderInfoController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("OrderInfoController");

    @Autowired
    private OrderInfoLogic orderInfoLogic;

    @RequestMapping("/orderInfoView.dhtml")
    public String init() {
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        tblOrderInfo.setId(Integer.parseInt(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblOrderInfo = orderInfoLogic.queryOrderInfo(tblOrderInfo);
            logger.info("查询结果:tblOrderInfo:" + (tblOrderInfo != null ? JSONObject.fromObject(tblOrderInfo).toString() : "null"));
            Map result = new HashMap();
            Map tblOrderInfo_map = afterLogic(request, JSONObject.fromObject(tblOrderInfo));
            result.put("tblOrderInfo", tblOrderInfo_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblOrderInfo tblOrderInfo = new TblOrderInfo();
            try {
                MapObjTransUtils.mapToObj(tblOrderInfo, paramMap);
            } catch (Exception e) {
            ...
            logger.info("传入参数:tblOrderInfo:" + JSONObject.fromObject(tblOrderInfo).toString());
            int count = orderInfoLogic.insertOrderInfo(tblOrderInfo);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblOrderInfo:" + JSONObject.fromObject(tblOrderInfo).toString());
            int count = orderInfoLogic.updateOrderInfo(tblOrderInfo);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblOrderInfo tblOrderInfo = new TblOrderInfo();
                tblOrderInfo.setId(Integer.parseInt(s));// 联合主键用逗号分割
                try {
                    count += orderInfoLogic.deleteOrderInfo(tblOrderInfo);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = orderInfoLogic.queryPageOrderInfo(paramMap);
