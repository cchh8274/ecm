...
import cn.kanmars.ecm.logic.UnionDemoSlaveaInfoLogic;
 ...
 */
@Controller
@RequestMapping("/unionDemoSlaveaInfo")
public class UnionDemoSlaveaInfoController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("UnionDemoSlaveaInfoController");

    @Autowired
    private UnionDemoSlaveaInfoLogic unionDemoSlaveaInfoLogic;

    @RequestMapping("/unionDemoSlaveaInfoView.dhtml")
    public String init() {
        return "uniondemoslaveainfo/uniondemoslaveainfo";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        tblUnionDemoSlaveaInfo.setDemoIda(Long.parseLong(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblUnionDemoSlaveaInfo = unionDemoSlaveaInfoLogic.queryUnionDemoSlaveaInfo(tblUnionDemoSlaveaInfo);
            logger.info("查询结果:tblUnionDemoSlaveaInfo:" + (tblUnionDemoSlaveaInfo != null ? JSONObject.fromObject(tblUnionDemoSlaveaInfo).toString() : "null"));
            Map result = new HashMap();
            Map tblUnionDemoSlaveaInfo_map = afterLogic(request, JSONObject.fromObject(tblUnionDemoSlaveaInfo));
            result.put("tblUnionDemoSlaveaInfo", tblUnionDemoSlaveaInfo_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            try {
                ...
                tblUnionDemoSlaveaInfo.setDemoIda(Long.parseLong(pk0));// 联合主键用逗号分割
                String pk1 = SysSequenceUtils.generateStringId("tbl_demo_info");
            } catch (Exception e) {
                logger.error("对象转化异常", e);
            }
            logger.info("传入参数:tblUnionDemoSlaveaInfo:" + JSONObject.fromObject(tblUnionDemoSlaveaInfo).toString());
            int count = unionDemoSlaveaInfoLogic.insertUnionDemoSlaveaInfo(tblUnionDemoSlaveaInfo);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblUnionDemoSlaveaInfo:" + JSONObject.fromObject(tblUnionDemoSlaveaInfo).toString());
            int count = unionDemoSlaveaInfoLogic.updateUnionDemoSlaveaInfo(tblUnionDemoSlaveaInfo);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblUnionDemoSlaveaInfo tblUnionDemoSlaveaInfo = new TblUnionDemoSlaveaInfo();
                tblUnionDemoSlaveaInfo.setDemoIda(Long.parseLong(s));// 联合主键用逗号分割
                try {
                    count += unionDemoSlaveaInfoLogic.deleteUnionDemoSlaveaInfo(tblUnionDemoSlaveaInfo);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = unionDemoSlaveaInfoLogic.queryPageUnionDemoSlaveaInfo(paramMap);
