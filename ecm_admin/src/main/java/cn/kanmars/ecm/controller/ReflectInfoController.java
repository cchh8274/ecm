...
import cn.kanmars.ecm.logic.ReflectInfoLogic;
 ...
 */
@Controller
@RequestMapping("/reflectInfo")
public class ReflectInfoController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("ReflectInfoController");

    @Autowired
    private ReflectInfoLogic reflectInfoLogic;

    @RequestMapping("/reflectInfoView.dhtml")
    public String init() {
        return "reflectinfo/reflectinfo";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        TblReflectInfo tblReflectInfo = new TblReflectInfo();
        tblReflectInfo.setId(Integer.parseInt(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblReflectInfo = reflectInfoLogic.queryReflectInfo(tblReflectInfo);
            logger.info("查询结果:tblReflectInfo:" + (tblReflectInfo != null ? JSONObject.fromObject(tblReflectInfo).toString() : "null"));
            Map result = new HashMap();
            Map tblReflectInfo_map = afterLogic(request, JSONObject.fromObject(tblReflectInfo));
            result.put("tblReflectInfo", tblReflectInfo_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblReflectInfo tblReflectInfo = new TblReflectInfo();
            try {
                MapObjTransUtils.mapToObj(tblReflectInfo, paramMap);
            } catch (Exception e) {
            ...
            logger.info("传入参数:tblReflectInfo:" + JSONObject.fromObject(tblReflectInfo).toString());
            int count = reflectInfoLogic.insertReflectInfo(tblReflectInfo);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblReflectInfo:" + JSONObject.fromObject(tblReflectInfo).toString());
            int count = reflectInfoLogic.updateReflectInfo(tblReflectInfo);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblReflectInfo tblReflectInfo = new TblReflectInfo();
                tblReflectInfo.setId(Integer.parseInt(s));// 联合主键用逗号分割
                try {
                    count += reflectInfoLogic.deleteReflectInfo(tblReflectInfo);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = reflectInfoLogic.queryPageReflectInfo(paramMap);
