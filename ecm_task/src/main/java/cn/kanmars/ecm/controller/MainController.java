...
import cn.kanmars.ecm.utils.GlobalTaskInfoUtils;
import cn.kanmars.ecm.utils.BasisConstants;
import cn.kanmars.ecm.utils.TaskTrigerUtils;
...
public class MainController {
    ...
    public ModelAndView Main(HttpServletRequest request,HttpServletResponse response) throws UnsupportedEncodingException {
        ...
        if(cookies!=null)for(Cookie cookie : cookies){
            if(cookie.getName().equals("ecm-taskadmin")){
        ...
        if(!isLogin&&s.equals("login")){
            ...
            if(userName.equals("admin")&&password.equals(MainSecurity.decode("axgh7wnjjRGqOMwPFYz4Qw=="))){
                ...
                Cookie ck = new Cookie("ecm-taskadmin",MainSecurity.encode(password));
        ...
        if(isLogin&&(StringUtils.isNotEmpty(s)&&s.equals("logout"))){
            Cookie ck = new Cookie("ecm-taskadmin","out");
            ck.setMaxAge(0);
            response.addCookie(ck);
            return mv;
        }
