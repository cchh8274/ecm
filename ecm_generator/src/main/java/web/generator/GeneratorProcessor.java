...
public class GeneratorProcessor extends AbstractProcessor {
    ...
    public void exec_a_01_selectDBINFO(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) {
        ...
        try {
            while((temp=br.readLine())!=null){
                ...
                if(temp.startsWith("ecm.jdbc.dbtype=")){
                    dbtype = temp.substring(temp.indexOf("=")+1);
                }
                if(temp.startsWith("ecm.jdbc.driverClass=")){
                    driver_class = temp.substring(temp.indexOf("=")+1);
                }
                if(temp.startsWith("ecm.jdbc.url=")){
                    url = temp.substring(temp.indexOf("=")+1);
                }
                if(temp.startsWith("ecm.jdbc.username=")){
                    username = temp.substring(temp.indexOf("=")+1);
                }
                if(temp.startsWith("ecm.jdbc.schema=")){
                    schema = temp.substring(temp.indexOf("=")+1);
                }
                if(temp.startsWith("ecm.jdbc.password=")){
    ...
    public void exec_a_02_buildFiles(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) {
        ...
        String alterCommentFlg = httpServletRequest.getParameter("alterCommentFlg");
        if(StringUtils.isEmpty(alterCommentFlg)){
            alterCommentFlg = "true";
        }
        ...
        try {
            ...
            //在页面通过base64对JSONARRAY中的value进行了转码
            //位于代码generator.html中
            for(JSONObject json : (List<JSONObject>)buildParamJson){
                for(Object o :json.entrySet()){
                    Map.Entry<String,String> e = (Map.Entry<String,String>)o;
                    if(e.getValue().startsWith("BASE64()")){
                        json.put(e.getKey(),URLDecoder.decode(new String(new BASE64Decoder().decodeBuffer(e.getValue().substring(8)),"utf-8"),"utf-8"));
                    }
                }
            }
            ...
            if(!StringUtils.isEmpty(driver_class)&&!StringUtils.isEmpty(url)){
                try {
                    ...
                    final String finalAlterCommentFlg = alterCommentFlg;
                    ...
                    g.exec(dbtype,basePath,driver_class,url,username,password,schema,new String[]{},new TableInfoMetaCreater(){
                        {
                            ...
                            if(finalAlterCommentFlg.equals("true")){
                                //输出COMMENT语句
                            ...
                            for(JSONObject json : (List<JSONObject>)buildParamJson){
                                ...
                                }else if(json.getString("type").equals("blob")){
                                    columnJSON = BlobModel.getInstance(
                                            json.getString("power"),
                                            json.getString("notEmptyMsg"),
                                            Integer.parseInt(json.getString("min")),
                                            Integer.parseInt(json.getString("max")),
                                            json.getString("strLengthMsg")).getModelJSON();
                                }else if(json.getString("type").equals("file")){
                                    columnJSON = FileModel.getInstance(
                                            json.getString("power"),
                                            json.getString("notEmptyMsg")).getModelJSON();
                                }
                                SQLTYPE sqltype = SQLTYPE.getSqlType(json.getString("sqlType"));
                                if(sqltype == null){
                                    throw new RuntimeException("不支持的数据库类型["+json.getString("sqlType")+"]");
                                }
                                this.addColumn(
                                        finalTableName,
                                ...
                                //如果alterComment标志为true则输出COMMENT语句
                                if(finalAlterCommentFlg.equals("true")){
                                    try{
                                        if(finalDbType.equals("MYSQL")){
                                            if(sqltype.equals(SQLTYPE.TINYBLOB)||sqltype.equals(SQLTYPE.BLOB)||sqltype.equals(SQLTYPE.MEDIUMBLOB)||sqltype.equals(SQLTYPE.LONGBLOB)){
                                                //MYSQL 中BLOB系列类型不支持长度指定
                                                sb.append("ALTER TABLE `"+finalTableName+"` MODIFY COLUMN `"+json.getString("column")+"` "+sqltype.toString()+" " +(json.getString("isNullAble").equals("YES")|json.getString("isNullAble").equals("true")?"NULL":"NOT NULL") +" COMMENT '"+json.getString("columnName")+ ":" + columnJSON.toString()+"';").append("\r\n");
                                            }else{
                                        ...
                                        if(finalDbType.equals("DB2")){
                                            sb.append("COMMENT ON COLUMN "+finalTableName+"."+json.getString("column")+" is '"+json.getString("columnName")+":"+columnJSON.toString() + "';").append("\r\n");
                                        }
                                    }catch (Exception e){
                                        System.out.println("列["+finalTableName+"."+json.getString("column")+"]生成altercomment错误，此错误可忽略" + sb.toString());
                                    }
                                }
                            }
                        }
