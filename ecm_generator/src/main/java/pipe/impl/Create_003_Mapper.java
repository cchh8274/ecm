...
public class Create_003_Mapper implements Pipe {
    ...
    public void exec(Context c) throws Exception {
        ...
        for(int i=0;i<count;i++){
            ...
            if(isPrimaryKey){
                //因Mybatis对数据库的兼容问题，sqlite并不支持resultSet.getBlob()方法，所以此处不返回"BLOB"
                //在不返回"BLOB"的情况下，mybatis的TypeHandlerRegistry中会直接通过new ByteArrayTypeHandler()来解析数据
                //则可以通过resultSet.getBytes()来获取到真实的数据
                sb.append("            <idArg column=\""+column_name
                        +"\" javaType=\""+TypeUtils.transDBType2MapperJavaType(column_type, column_size, decimal_digits)+"\" "
                        +((TypeUtils.transDBType2MapperJDBCType(column_type, column_size, decimal_digits).equals("BLOB"))?"":"jdbcType=\""+TypeUtils.transDBType2MapperJDBCType(column_type, column_size, decimal_digits)+"\" ")
                        +"/>").append("\r\n");
            }
        }
        //增加普通字段
